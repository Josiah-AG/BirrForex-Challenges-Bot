...
export class Bot {
  ...
  private async setupBotCommands() {
    ...
    await this.bot.telegram.setMyCommands([
      ...
      { command: 'requestsubmission', description: 'Open submission window (48h deadline)' },
  ...
  private setupHandlers() {
    ...
    this.bot.command('requestsubmission', (ctx) => tradingAdminHandler.requestSubmission(ctx));
        ...
        if (data.startsWith('gate_approve_')) {
          ...
          if (pending.type === 'create') {
            ...
            if (result.success) {
              ...
              } catch (emailErr) { console.error('Challenge-approved email failed:', emailErr); }
            } else {
