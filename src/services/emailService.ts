...
class EmailService {
  ...
  async sendRegistrationConfirmation(to: string, data: {
    ...
    try {
      const content = `
        ...
        <div style="background: #1a2740; border-radius: 10px; padding: 16px 20px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 20px;">
          <p style="color: #6366f1; font-size: 15px; font-weight: 700; margin: 0 0 2px;">${data.challengeTitle}</p>
          <p style="color: #7a8ba3; font-size: 12px; margin: 0;">Your spot is confirmed</p>
        </div>

        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 20px;">
          <tr><td style="padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); color: #7a8ba3; font-size: 13px;">Account</td><td style="padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); color: #ffffff; font-size: 13px; text-align: right; font-weight: 600;">${data.accountNumber}</td></tr>
          <tr><td style="padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); color: #7a8ba3; font-size: 13px;">Type</td><td style="padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); color: #ffffff; font-size: 13px; text-align: right; font-weight: 600;">${data.accountType.charAt(0).toUpperCase() + data.accountType.slice(1)}</td></tr>
          <tr><td style="padding: 10px 0; color: #7a8ba3; font-size: 13px;">Nickname</td><td style="padding: 10px 0; color: #ffffff; font-size: 13px; text-align: right; font-weight: 600;">${data.nickname}</td></tr>
        </table>
  ...
  async sendBalanceWarning(to: string, data: {
    ...
    try {
      const content = `
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 20px;">
          <tr><td style="padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); color: #7a8ba3; font-size: 13px;">Your Balance</td><td style="padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); color: #d97706; font-size: 14px; text-align: right; font-weight: 700;">${data.currentBalance}</td></tr>
          <tr><td style="padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); color: #7a8ba3; font-size: 13px;">Challenge Limit</td><td style="padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); color: #ffffff; font-size: 13px; text-align: right; font-weight: 600;">${data.limit}</td></tr>
          <tr><td style="padding: 10px 0; color: #7a8ba3; font-size: 13px;">Challenge Starts</td><td style="padding: 10px 0; color: #ffffff; font-size: 13px; text-align: right; font-weight: 600;">${data.startDate}</td></tr>
        </table>

        <div style="background: rgba(245,158,11,0.1); border-radius: 8px; padding: 14px 16px; border: 1px solid rgba(245,158,11,0.3);">
  ...
  async sendDisqualification(to: string, data: {
    ...
    try {
      const content = `
        ...
        <div style="background: rgba(239,68,68,0.1); border-radius: 8px; padding: 14px 16px; border: 1px solid rgba(239,68,68,0.3); margin-bottom: 20px;">
          <p style="color: #7a8ba3; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; margin: 0 0 6px; font-weight: 600;">Reason</p>
  ...
  async sendChallengeStarted(to: string, data: {
    ...
    try {
      const content = `
        ...
        <div style="background: rgba(34,197,94,0.1); border-radius: 10px; padding: 16px 20px; border: 1px solid rgba(34,197,94,0.3); margin-bottom: 20px;">
          <p style="color: #86efac; font-size: 15px; font-weight: 700; margin: 0 0 4px;">${data.challengeTitle}</p>
          <p style="color: #94a3b8; font-size: 13px; margin: 0;">Ends on <strong>${data.endDate}</strong></p>
        </div>
  ...
  async sendChallengeEnded(to: string, data: {
    ...
    try {
      const content = `
        ...
        <div style="background: rgba(99,102,241,0.1); border-radius: 10px; padding: 16px 20px; border: 1px solid rgba(99,102,241,0.3); margin-bottom: 20px;">
  ...
  async sendGeneric(to: string, subject: string, bodyContent: string): Promise<boolean> {
    if (!this.isConfigured()) return false;
    try {
      const content = `<div style="color: #e2e8f0; font-size: 14px; line-height: 1.6;">${bodyContent}</div>`;
