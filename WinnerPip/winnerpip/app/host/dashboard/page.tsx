            ...
            <div className="p-5 space-y-4">
              ...
              {(selectedParticipant.isDisqualified || selectedParticipant.disqualified) ? (
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="bg-white/5 rounded-xl p-3 text-center"><p className="text-[10px] text-gray-500">{(selectedChallenge as any)?.deposit_mode && (selectedChallenge as any).deposit_mode !== 'fixed' ? 'Growth' : 'Balance'}</p><p className="text-2xl font-bold text-white">{(selectedChallenge as any)?.deposit_mode && (selectedChallenge as any).deposit_mode !== 'fixed' ? `${Number(selectedParticipant.growthPercent || 0) >= 0 ? '↑' : '↓'} ${Math.abs(Number(selectedParticipant.growthPercent || 0)).toFixed(1)}%` : cur(selectedParticipant.adjustedBalance, selectedParticipant.isCent)}</p>{(selectedChallenge as any)?.deposit_mode && (selectedChallenge as any).deposit_mode !== 'fixed' && <p className="text-[10px] text-gray-500 mt-0.5">{cur(selectedParticipant.adjustedBalance, selectedParticipant.isCent)}</p>}</div>
