                  ...
                  <tbody>{leaderboard.length === 0 ? <tr><td colSpan={10} className="py-8 text-center text-gray-500">No leaderboard data yet — will populate after updates and evaluation</td></tr> : leaderboard.filter((e: any) => leaderboardCategory === 'all' || e.accountType === leaderboardCategory).map((e: any) => {
                    ...
                    <tr key={e.rank || e.nickname} className={`border-b border-white/5 hover:bg-white/5 cursor-pointer ${e.isDisqualified ? "opacity-50 bg-loss/10" : (e.isWithdrawn || e.isBlown) ? "opacity-40 bg-loss/5" : eIsWinner ? "bg-profit/15" : eIsAboveTarget ? "bg-profit/5" : ""}`} onClick={() => setSelectedParticipant(e)}>
                      ...
                      <td className="py-3 px-4 text-right">
                        <p className={`text-sm font-bold ${e.isDisqualified ? "text-loss" : e.isWithdrawn ? "text-gray-500" : eIsWinner ? "text-profit" : eIsAboveTarget ? "text-profit/80" : "text-white"}`}>{e.isDisqualified ? "DQ" : e.isWithdrawn ? "Exited" : eDepositMode !== 'fixed' ? (<span className={Number(e.growthPercent || 0) >= 0 ? "text-profit" : "text-loss"}>{Number(e.growthPercent || 0) >= 0 ? '↑' : '↓'} {Number(e.growthPercent || 0) >= 0 ? '+' : '-'}{Math.abs(Number(e.growthPercent || 0)).toFixed(1)}%</span>) : e.isCent ? `${(Number(e.adjustedBalance) - (e.totalWithdrawn || 0)).toFixed(2)}¢` : `$${(Number(e.adjustedBalance) - (e.totalWithdrawn || 0)).toFixed(2)}`}</p>
            ...
            <div className="p-5 space-y-4">
              ...
              {(selectedParticipant.isDisqualified || selectedParticipant.disqualified) ? (
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="bg-white/5 rounded-xl p-3 text-center"><p className="text-[10px] text-gray-500">{(selectedChallenge as any)?.deposit_mode && (selectedChallenge as any).deposit_mode !== 'fixed' ? 'Growth' : 'Balance'}</p><p className={`text-2xl font-bold ${(selectedChallenge as any)?.deposit_mode && (selectedChallenge as any).deposit_mode !== 'fixed' ? (Number(selectedParticipant.growthPercent || 0) >= 0 ? 'text-profit' : 'text-loss') : 'text-white'}`}>{(selectedChallenge as any)?.deposit_mode && (selectedChallenge as any).deposit_mode !== 'fixed' ? <>{Number(selectedParticipant.growthPercent || 0) >= 0 ? '↑' : '↓'} {Number(selectedParticipant.growthPercent || 0) >= 0 ? '+' : '-'}{Math.abs(Number(selectedParticipant.growthPercent || 0)).toFixed(2)}%</> : cur(selectedParticipant.adjustedBalance, selectedParticipant.isCent)}</p>{(selectedChallenge as any)?.deposit_mode && (selectedChallenge as any).deposit_mode !== 'fixed' ? <><p className="text-[10px] text-gray-500 mt-0.5">Start: {cur(selectedParticipant.actualStartingBalance ?? selectedParticipant.adjustedBalance, selectedParticipant.isCent)}</p><p className="text-[10px] text-gray-500">Now: {cur(selectedParticipant.adjustedBalance, selectedParticipant.isCent)}</p></> : null}</div>
            ...
            {createStep === 3 && (
              <div>
                <div className="space-y-2 mb-5 text-xs">
                  ...
                  {createForm.type === 'hybrid' && createForm.split_category_settings ? (
                    <>
                      <div className="p-3 rounded-lg bg-blue-500/5 border border-blue-500/10 mb-2">
                        <p className="text-[10px] text-blue-400 font-bold uppercase mb-1.5">Demo Rules</p>
                        {createRulesDemo.rules_enabled.max_lot_size && <div className="flex justify-between py-1"><span className="text-gray-500">Max Lot Size</span><span className="text-white">{createRulesDemo.max_lot_size}</span></div>}
                        {createRulesDemo.rules_enabled.max_open_trades && <div className="flex justify-between py-1"><span className="text-gray-500">Max Open Trades</span><span className="text-white">{createRulesDemo.max_open_trades}</span></div>}
                        {createRulesDemo.rules_enabled.pair_limit && <div className="flex justify-between py-1"><span className="text-gray-500">Pair Limit</span><span className="text-white">{createRulesDemo.pair_limit}</span></div>}
                        {createRulesDemo.rules_enabled.stop_loss_required && <div className="flex justify-between py-1"><span className="text-gray-500">Max Risk</span><span className="text-white">{createRulesDemo.max_risk_mode === 'percentage' ? `${createRulesDemo.max_risk_percent}%` : `$${createRulesDemo.max_risk_dollars}`}</span></div>}
                        {createRulesDemo.rules_enabled.daily_loss_cap && <div className="flex justify-between py-1"><span className="text-gray-500">Daily Loss Cap</span><span className="text-white">{createRulesDemo.daily_loss_mode === 'percentage' ? `${createRulesDemo.daily_loss_percent}%` : `$${createRulesDemo.daily_loss_cap}`}</span></div>}
                      ...
                      <div className="p-3 rounded-lg bg-profit/5 border border-profit/10">
                        <p className="text-[10px] text-profit font-bold uppercase mb-1.5">Real Rules</p>
                        {createRulesReal.rules_enabled.max_lot_size && <div className="flex justify-between py-1"><span className="text-gray-500">Max Lot Size</span><span className="text-white">{createRulesReal.max_lot_size}</span></div>}
                        {createRulesReal.rules_enabled.max_open_trades && <div className="flex justify-between py-1"><span className="text-gray-500">Max Open Trades</span><span className="text-white">{createRulesReal.max_open_trades}</span></div>}
                        {createRulesReal.rules_enabled.pair_limit && <div className="flex justify-between py-1"><span className="text-gray-500">Pair Limit</span><span className="text-white">{createRulesReal.pair_limit}</span></div>}
                        {createRulesReal.rules_enabled.stop_loss_required && <div className="flex justify-between py-1"><span className="text-gray-500">Max Risk</span><span className="text-white">{createRulesReal.max_risk_mode === 'percentage' ? `${createRulesReal.max_risk_percent}%` : `$${createRulesReal.max_risk_dollars}`}</span></div>}
                        {createRulesReal.rules_enabled.daily_loss_cap && <div className="flex justify-between py-1"><span className="text-gray-500">Daily Loss Cap</span><span className="text-white">{createRulesReal.daily_loss_mode === 'percentage' ? `${createRulesReal.daily_loss_percent}%` : `$${createRulesReal.daily_loss_cap}`}</span></div>}
