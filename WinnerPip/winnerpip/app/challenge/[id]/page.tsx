...
import { TrendingUp, Trophy, AlertTriangle, Target, Activity, ArrowLeft, FileText, Clock, ChevronDown, ChevronUp, Shield, Award, Hash, Key, Loader2, MessageCircle, ArrowRight, X } from "lucide-react";
...
export default function ChallengeDashboard() {
  ...
  useState(() => { if (typeof window !== "undefined" && localStorage.getItem("wp_token")) setIsLoggedIn(true); });
  ...
  const handleLogin = async () => {
    ...
    if (loginAccount === "87654321" && loginPassword === "demo123") {
      localStorage.setItem("wp_token", "demo_token");
      setIsLoggedIn(true); setShowLogin(false); setLoginLoading(false); return;
    }
    try {
      ...
      if (res.ok) { const d = await res.json(); localStorage.setItem("wp_token", d.token); setIsLoggedIn(true); setShowLogin(false); }
      else setLoginError("This account and password are not registered. Check credentials or register first.");
    } catch { setLoginError("This account and password are not registered. Check credentials or register first."); }
  ...
  // ==================== PLACEHOLDER DATA ====================
  const challenge = { id: params.id, title: "Challenge 15 — Hybrid (Demo & Real)", status: "active", daysLeft: 7, startingBalance: 30, targetBalance: 60 };
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="hidden sm:block">
                  ...
                  <p className="text-xs text-gray-500">{myStats.nickname} • #{isLoggedIn ? "87654321" : ""}</p>
                </div>
              </div>
            </div>
          ...
          <div className="max-w-md mx-auto py-12">
            <div className="glass rounded-3xl border border-white/10 p-8">
              ...
              <div className="space-y-4">
                <div><label className="flex items-center gap-2 text-sm font-medium text-gray-300 mb-2"><Hash size={14} className="text-royal" />Account Number</label><Input type="text" inputMode="numeric" placeholder="e.g., 87654321" value={loginAccount} onChange={(e) => setLoginAccount(e.target.value.replace(/\D/g, ""))} /></div>
              ...
              <p className="text-xs text-gray-500 mt-4 text-center">Demo: <code className="text-gray-400">87654321</code> / <code className="text-gray-400">demo123</code></p>
            </div>
          </div>
        )}
          ...
          <div className="glass rounded-2xl max-w-lg w-full max-h-[85vh] overflow-y-auto border border-royal/30" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-5 space-y-3">
              ...
              <div className="flex flex-col gap-2 mt-4">
                <a href="https://example.com/challenge-15-rules.pdf" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 p-3 rounded-xl bg-royal/20 border border-royal/30 hover:bg-royal/30 text-royal font-semibold text-sm transition-all">
                  <FileText size={16} /> Download Rules PDF
                </a>
                <a href="https://example.com/challenge-15-guide" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 p-3 rounded-xl bg-gold/20 border border-gold/30 hover:bg-gold/30 text-gold font-semibold text-sm transition-all">
