            ...
            {foundUser && (
              <div className="glass rounded-2xl border border-white/10 overflow-hidden">
                ...
                <div className="p-5 border-t border-white/10 space-y-2">
                  ...
                  <button onClick={() => { const data = foundUser; if(!data.recentTrades?.length){ alert("No trades to export"); return; } const rows=["Ticket\tOpen Time\tClose Time\tType\tVolume\tSymbol\tOpen Price\tClose Price\tS/L\tT/P\tCommission\tSwap\tProfit"]; data.recentTrades.forEach((t:any)=>rows.push(`${t.ticket||""}\t${t.openTime||""}\t${t.closeTime||t.date||""}\t${t.type||""}\t${t.volume||""}\t${t.symbol||""}\t${t.openPrice||""}\t${t.closePrice||""}\t${t.stopLoss||""}\t${t.takeProfit||""}\t${t.commission||"0.00"}\t${t.swap||"0.00"}\t${t.profit||"0.00"}`)); const tsv=rows.join("\n"); const blob=new Blob([tsv],{type:"text/tab-separated-values"}); const url=URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url; a.download=`${data.accountNumber}_MT5_History.csv`; a.click(); }} className="w-full flex items-center justify-center gap-2 p-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 text-gray-300 font-semibold transition-all text-sm"><FileText size={16} />Export MT5 Trade History</button>
                </div>
              </div>
            )}
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-[900px]">
                      ...
                      <tbody>{participantsList.map((p) => (
                        <tr key={p.id} className={`border-b border-white/5 hover:bg-white/5 cursor-pointer transition-colors ${p.disqualified ? "opacity-50 bg-loss/5" : ""}`} onClick={async () => { const q = p.nickname || p.accountNumber; setSearchQuery(q); try { const _api = process.env.NEXT_PUBLIC_API_URL || "https://api.winnerpip.com"; const _path = process.env.NEXT_PUBLIC_ADMIN_PATH || ""; const res = await fetch(`${_api}/api/admin/${_path}/challenge/${selectedChallengeId}/find-user?q=${encodeURIComponent(q)}`); if (res.ok) { const data = await res.json(); if (data.user) setFoundUser(data.user); } } catch {} }}>
