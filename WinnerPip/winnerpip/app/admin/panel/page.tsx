            ...
            {foundUser && (
              <div className="glass rounded-2xl border border-white/10 overflow-hidden">
                ...
                <div className="p-5 border-t border-white/10 space-y-2">
                  ...
                  <button onClick={async () => { const data = foundUser; if(!data.id){ alert("No user data"); return; } try { const _api = process.env.NEXT_PUBLIC_API_URL || "https://api.winnerpip.com"; const _path = process.env.NEXT_PUBLIC_ADMIN_PATH || ""; const res = await fetch(`${_api}/api/admin/${_path}/challenge/${selectedChallengeId}/user-trades-mt5?registration_id=${data.id}`); if (!res.ok) { alert("Failed to fetch trades"); return; } const result = await res.json(); if (!result.positions?.length) { alert("No trades found"); return; } const fmt = (d:string) => { if(!d) return ""; return d.replace("T"," ").replace("Z","").substring(0,19).replace(/-/g,"."); }; const rows = ["Ticket\tOpen Time\tClose Time\tType\tVolume\tSymbol\tOpen Price\tClose Price\tS/L\tT/P\tCommission\tSwap\tProfit"]; result.positions.forEach((t:any) => rows.push(`${t.Position}\t${fmt(t.Time)}\t${fmt(t["Close Time"])}\t${t.Type}\t${t.Volume}\t${t.Symbol}\t${t.Price}\t${t["Close Price"]}\t${t["S / L"]}\t${t["T / P"]}\t${t.Commission}\t${t.Swap}\t${t.Profit}`)); const tsv = rows.join("\n"); const blob = new Blob([tsv], {type:"text/tab-separated-values"}); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `${data.accountNumber}_MT5_History.csv`; a.click(); } catch { alert("Export failed"); } }} className="w-full flex items-center justify-center gap-2 p-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 text-gray-300 font-semibold transition-all text-sm"><FileText size={16} />Export MT5 Trade History</button>
                </div>
              </div>
            )}
      ...
      <div className="glass rounded-2xl border border-white/10 p-6 space-y-6">
        ...
        <div className="border-t border-white/10 pt-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            ...
            <button onClick={async () => { try { const _api = process.env.NEXT_PUBLIC_API_URL || "https://api.winnerpip.com"; const _path = process.env.NEXT_PUBLIC_ADMIN_PATH || ""; const res = await fetch(`${_api}/api/admin/${_path}/challenge/${selectedChallengeId}/export-evaluation`); if (res.ok) { const data = await res.json(); const csv = convertToCSV(data.evaluation); const blob = new Blob([csv], { type: "text/csv" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `challenge_${selectedChallengeId}_evaluation.csv`; a.click(); setMsg("✅ Evaluation exported"); } } catch { setMsg("❌ Export failed"); } }} className="p-2 sm:p-2.5 rounded-lg bg-profit/10 border border-profit/30 text-profit text-[10px] sm:text-xs font-semibold hover:bg-profit/20 transition-all">📋 Evaluation</button>
          </div>
        </div>
