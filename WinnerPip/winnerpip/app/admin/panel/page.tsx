          ...
          <div className="space-y-6">
            ...
            {foundUser && (
              <div className="glass rounded-2xl border border-white/10 overflow-hidden">
                ...
                <div className="p-5 border-t border-white/10"><button onClick={() => { const data = foundUser; const rows = [["Field","Value"],["Nickname",data.nickname],["Username",data.username],["Email",data.email],["Account",data.accountNumber],["Type",data.accountType],["Server",data.server],["Balance",data.balance],["Qualified Profit",data.qualifiedProfit],["Gross Profit",data.grossProfit],["Trades",data.totalTrades],["Flagged",data.flaggedTrades],["Active Days",data.activeDays],["Rank",data.rank],["Registered",data.registeredAt],["Last Pull",data.lastPull]]; if(data.recentTrades?.length){rows.push([""],[" --- TRADES ---"]);rows.push(["Symbol","Type","Volume","Profit","Qualified"]);data.recentTrades.forEach((t:any)=>rows.push([t.symbol,t.type,t.volume,t.profit,t.isQualified?"Yes":"No"]));} const csv=rows.map((r:any)=>r.join(",")).join("\n"); const blob=new Blob([csv],{type:"text/csv"}); const url=URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url; a.download=`${data.nickname}_${data.accountNumber}.csv`; a.click(); }} className="w-full flex items-center justify-center gap-2 p-4 rounded-xl bg-royal/20 border border-royal/30 hover:bg-royal/30 text-royal font-semibold transition-all"><FileText size={18} />Export User Data &amp; MT5 Trades (CSV)</button></div>
              </div>
            )}
